...
;; the position of the baseball at time t.

(define position
  (lambda (a v u t)
    (define t-squared (square t))
    (define v-t (* v t))
    (define half-a (* a (/ 1 2)))
    (+ (* half-a t-squared) (+ v-t u))))
...
;; Problem 2
;; Given the coefficients of the quadratic equation az^2 + bz + c = 0, find the roots
;; x = (-b +/- sqrt(b^2 - 4*a*ac))/2a

(define b-squared-minus-four-ac
  (lambda (a b c)
  (define b-squared (square b))
  (define four-ac (* 4 (* a c)))
  (- b-squared four-ac)))


(define root1
  (lambda (a b c)
    (define bsq (b-squared-minus-four-ac a b c))
    (define sqrt-part (sqrt bsq))
    (define 2a (* 2 a))
    (define negb (- b))
    (define numer (+ negb sqrt-part))
    (/ numer 2a)))
...
;; Problem 3
;; given an initial upward velocity and elevation, what is the time in the air
;; 0 = -0.5at^2 + vt + u

(define time-to-impact
  (lambda (vertical-velocity elevation)
    (define a (- (* (/ 1 2) gravity)))
    (define t1 (root1 a vertical-velocity elevation))
    (define t2 (root2 a vertical-velocity elevation))
...
(define time-to-impact
  (lambda (vertical-velocity elevation target-elevation)
    (define a (- (* (/ 1 2) gravity)))
    (define t1 (root1 a vertical-velocity (- elevation target-elevation)))
    (define t2 (root2 a vertical-velocity (- elevation target-elevation)))
...
(define travel-distance-simple
  (lambda (elevation velocity angle)
    (define xvel (* velocity (cos (deg2rad angle))))
    (define yvel (* velocity (sin (deg2rad angle))))
    (define t_impact (time-to-impact yvel elevation 0))
    (define dist (* xvel t_impact))
    dist))
...
(define find-best-angle
  (lambda (velocity elevation)
    ...
    ;; returns the max of the two angles
    (define best-angle (lambda (angle1 angle2)
      (define d1 (travel-distance-simple elevation velocity angle1))
      (define d2 (travel-distance-simple elevation velocity angle2))
      (if (> d1 d2)
	  angle1
	  angle2)))
    
    ;; loops until angle > 90
    (define loop (lambda (angle best-so-far)
      (define next-angle (+ angle alpha-increment))
      (define new-best-angle (best-angle angle best-so-far))

      (if (> angle 90)
	  best-so-far
	  (loop next-angle new-best-angle))))
...
(define integrate
  (lambda (x0 y0 u0 v0 dt g m beta)
    ...
    (define loop (lambda (x y u v)
      (define speed (sqrt (+ (square u) (square v))))
      (define speed_beta_term (* (/ 1 m) speed beta))

      (define du (* (- speed_beta_term) u dt))
      (define dx (* u dt))
      (define u_new (+ u du))
      (define x_new (+ x dx))

      (define dv (* (- (+ speed_beta_term g)) dt))
      (define dy (* v dt))
      (define v_new (+ v dv))
      (define y_new (+ y dy))
	  ...
	  (loop x_new y_new u_new v_new))))

    (loop x0 y0 u0 v0)))

(define travel-distance
  (lambda (initial-velocity angle elevation)
    (define x0 0)
    (define v0 (* initial-velocity (sin (deg2rad angle))))
    (define u0 (* initial-velocity (cos (deg2rad angle))))
    (define dt 0.01)
    (define int_list (integrate x0 elevation u0 v0 dt gravity mass beta))
  (car (car int_list))))
...
(define find-throwing-angle
  (lambda (initial-velocity elevation distance receiver-height)

    (define loop (lambda (angle best-angle)
		   (define next-angle (+ angle 0.5))
		   (define best-dist (travel-distance initial-velocity best-angle elevation))
		   (define dist (travel-distance initial-velocity angle elevation))
		   (define best-dist-delta (abs (- best-dist distance)))
		   (define dist-delta (abs (- dist distance)))

		   (display next-angle)(newline)

		   (cond ((> angle 90) best-angle)
			    ((< best-dist-delta dist-delta) (loop next-angle best-angle))
			    (else (loop next-angle angle)))))
    (loop 0 0)))
